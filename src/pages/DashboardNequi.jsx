...
import FloatingWithdrawalCode from "../components/FloatingWithdrawalCodeNequi";
import Header from "../components/Header";

const DashboardNequi = () => {
    ...
    // Estados para el modal de retiro (solicitando solo la clave)
    ...
    // (Opcional) Obtener transacciones recientes, si tienes un endpoint para ello
    useEffect(() => {
    ...
    // Avanzar en el modal de retiro (solamente se pide la clave)
    ...
    // Generar el código de seguridad llamando al backend
    const generateSecurityCode = async () => {
        ...
        try {
            ...
            const res = await fetch("http://localhost:5000/api/cuentas/generar-codigo", {
                ...
                // Se envía el número de cuenta y la clave
            ...
            setWithdrawStep(2);
            setSuccess(true);
            // Mostrar el panel flotante con el código
    ...
    // Función para copiar el código (se usará en el panel flotante)
    ...
    return (
        <div className="relative min-h-screen overflow-x-hidden">
            {/* Background gradient y partículas animadas */}

            <div className="absolute inset-0 bg-gradient-to-br from-[#6a11cb] via-[#8e44ad] to-[#2c3e50] z-0"></div>

            <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-purple-800 to-purple-900 text-white shadow-lg pt-8">
            <Header className="z-50" />

                <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                    <div className="flex items-center">
                        <img src={nequiLogo || "/placeholder.svg"} alt="Nequi" className="h-10 mr-3" />
                        <h1 className="text-xl font-bold hidden md:block">Nequi Digital</h1>
                    </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-1">
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
                            <Card className="bg-white/90 backdrop-blur-md rounded-2xl shadow-lg overflow-hidden border-t-4 border-purple-400">
                                <div className="bg-gradient-to-r from-purple-700 to-purple-900 p-6 text-white">
                                    <div className="flex items-center mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-purple-200">Cliente Nequi</p>
                                        </div>
                                    </div>
                                </div>
                                <CardContent className="p-6">
                                    <div className="space-y-6">
                                        <div>
                                            <h3 className="text-sm text-gray-500 mb-1">Número de cuenta</h3>
                                        ...
                                        <div>
                                            <h3 className="text-sm text-gray-500 mb-1">Teléfono</h3>
                                            <p className="text-lg font-medium text-purple-900 flex items-center">
                                        ...
                                        <div>
                                            <h3 className="text-sm text-gray-500 mb-1">Tipo de cuenta</h3>
                                            <p className="text-lg font-medium text-purple-900 capitalize">{userData.tipo}</p>
                                        </div>
                                        <div className="pt-4 border-t border-gray-100">
                                            <h3 className="text-sm text-gray-500 mb-1">Saldo disponible</h3>
                                            <p className="text-3xl font-bold text-purple-900">{formatCurrency(userData.monto)}</p>
                                        </div>
                                        <Button
                                            onClick={handleWithdraw}
                                            className="w-full bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-semibold py-3 rounded-xl shadow-md hover:shadow-lg transition-all"
                                        >
                    ...
                    <div className="lg:col-span-2">
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}>
                            <Card className="bg-gradient-to-r from-purple-700 to-purple-900 text-white rounded-2xl shadow-lg overflow-hidden mb-6 p-2">
                                <CardContent className="p-6">
                                    <div className="flex flex-col md:flex-row justify-between items-center">
                                        <div>
                                            <h3 className="text-lg font-medium text-purple-200 mb-1">Saldo total</h3>
                                            <p className="text-4xl font-bold">{formatCurrency(userData.monto)}</p>
                                            <p className="text-sm text-purple-200 mt-1">Actualizado: {new Date().toLocaleDateString()}</p>
                                        </div>
                                        <div className="mt-4 md:mt-0 flex space-x-3">
                                            <Button
                                                onClick={handleWithdraw}
                                                className="bg-white text-purple-800 hover:bg-purple-50 font-medium px-4 py-2 rounded-xl shadow-md hover:shadow-lg transition-all"
                                            >
                                ...
                                <Card className="bg-white rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer pt-4">
                                    <CardContent className="p-4 flex flex-col items-center text-center">
                                        <div className="bg-blue-100 p-3 rounded-full mb-3">
                                            <FaCopy className="h-6 w-6 text-purple-800" />
                                        </div>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }}>
                            <Card className="bg-white/90 backdrop-blur-md rounded-2xl shadow-lg overflow-hidden">
                                            ...
                                            recentTransactions.map((transaction) => (
                                                <div key={transaction._id} className="p-4 hover:bg-gray-50 transition-colors">
                                                    <div className="flex justify-between items-center">
                                                        <div className="flex items-center">
                                                            <div className={`p-2 rounded-full mr-3 ${transaction.type === "Depósito" ? "bg-green-100" : transaction.type === "Retiro" ? "bg-red-100" : "bg-blue-100"}`}>
                                                                <FaMoneyBillWave className={`h-4 w-4 ${transaction.type === "Depósito" ? "text-green-600" : transaction.type === "Retiro" ? "text-red-600" : "text-blue-600"}`} />
                                                            </div>
                                                            <div>
                                                        ...
                                                        <div className="text-right">
                                                            <p className={`font-medium ${transaction.type === "Depósito" ? "text-green-600" : transaction.type === "Retiro" ? "text-red-600" : "text-blue-600"}`}>
                            ...
                            <Card className="bg-white rounded-3xl overflow-hidden shadow-2xl border border-purple-200">
                                <div className="bg-gradient-to-r from-purple-600 to-purple-800 p-6 relative">
                                    ...
                                    <div className="flex justify-center mt-4">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${withdrawStep === 1 ? "bg-purple-300 text-purple-900" : "bg-purple-800 text-white/70"}`}>
                                            1
                                        </div>
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ml-4 ${withdrawStep === 2 ? "bg-purple-300 text-purple-900" : "bg-purple-800 text-white/70"}`}>
                                            2
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="relative">
                                                    <Input
                                                        ...
                                                        className="w-full pr-10 rounded-xl border-purple-200 focus:border-purple-500 focus:ring focus:ring-purple-200 transition-all"
                                                        value={password}
                                        ...
                                        <div className="space-y-2 text-center pt-4">
                                            {success ? (
                                                <>
                                                    ...
                                                    <h2 className="text-xl font-bold text-gray-800 mb-2">¡Código generado con éxito!</h2>
                                                    <p className="text-gray-600 mb-6">
                                                        Usa este código en el cajero para retirar tu dinero.
                                                    </p>
                                                    <div className="bg-purple-50 p-4 rounded-xl border border-purple-200 mb-4">
                                                        <div className="flex justify-between items-center">
                                                            <div className="text-3xl font-bold tracking-wider text-purple-800">
                                                                {securityCode}
                                                            </div>
                                                            <button
                                    ...
                                    <div className={`flex ${withdrawStep === 1 ? "justify-end" : "justify-between"} mt-6`}>
                                        ...
                                        {withdrawStep === 1 ? (
                                            <Button
                                                ...
                                                disabled={loading}
                                                className="bg-gradient-to-r from-purple-600 to-purple-800 hover:from-purple-700 hover:to-purple-900 text-white font-semibold py-3 px-6 rounded-xl shadow-md hover:shadow-lg transition-all"
                                            >
            ...
            {/* Footer */}
            <footer className="relative z-10 bg-gradient-to-r from-purple-800 to-purple-900 text-white py-4 mt-12">
